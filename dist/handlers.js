...
const characterNameParagraph = document.getElementById("characterNameParagraph");
export function partClickHandler(part) {
...
export function partMouseOverHandler(part) {
    part.highlight();
}
export function partMouseOutHandler(part) {
    part.div.style.filter = "";
}
export function mainSwitchHandler(direction, pageNumber) {
    // edge cases
    if ((pageNumber === 0 && direction === "left") ||
...
export function characterSwitchHandler(direction, characterNumber) {
    direction === "left" ? characterNumber-- : characterNumber++;
    // inifite character's array switcher
    if (characterNumber < 0)
        characterNumber = characterArray.length - 1;
    if (characterNumber > characterArray.length - 1)
        characterNumber = 0;
...
export function partSwitchHandler(partType, direction) {
